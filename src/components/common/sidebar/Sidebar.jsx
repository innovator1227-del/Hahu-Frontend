...
const Sidebar = ({ isOpen, onClose, variant }) => {
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-50 h-screen flex flex-col shadow-2xl transition-all duration-500 ease-in-out ${isOpen ? "w-64 translate-x-0" : "-translate-x-full md:translate-x-0 md:w-22"}
        `}
      >
        {/* Header */}
        <div className="h-16 md:h-20 flex items-center px-5 border-b border-white/5 bg-transparent">
          <div className="flex items-center gap-3">
            {/* Logo ALWAYS visible */}
            <img
              src={Logo}
              alt="HAHU Market Logo"
              className={`w-10 h-10 rounded-full ring-2 ring-white/5 object-cover transition-all duration-500 ease-in-out `}
            />
